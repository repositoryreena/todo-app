document.addEventListener("DOMContentLoaded", () => {
  ...
  function saveCategories() {
    ...
    const categoryData = Array.from(categoryList.children).map(
      (categoryDiv) => {
        const categoryName = categoryDiv.querySelector("h3").textContent.trim();
        const cleanedCategoryName = categoryName.replace(/❌/g, "");

        const tasks = Array.from(
          categoryDiv.querySelectorAll(".task-list li")
        ).map((taskLi) => ({
  ...
  function addCategory(categoryName, tasks = []) {
    ...
    showAllBtn.addEventListener("click", () => {
      ...
      sortTasksByCompletionAndPriority(category); // Sort by completion and priority
    });

    // Function to sort tasks by completion status (incomplete first) and then by priority
// Function to sort tasks by completion status (incomplete first) and then by priority
function sortTasksByCompletionAndPriority(category) {
  const taskUl = category.querySelector(".task-list");
  const tasks = Array.from(taskUl.children);

  // Sort incomplete tasks first, then sort by priority
  ...
  // Sort both incomplete and completed tasks by priority
  ...
  // Combine the tasks back together, with incomplete tasks first and completed tasks last
  const sortedTasks = [...incompleteTasks, ...completedTasks];

  // Re-append tasks in the new sorted order
  sortedTasks.forEach((task) => taskUl.appendChild(task));
  saveCategories(); // Save the updated order to localStorage
}
  ...
  function enableTaskDragging(taskUl) {
    taskUl.addEventListener("dragstart", (event) => {
      ...
      if (task && task.tagName === "LI") {
        ...
        event.dataTransfer.setData(
          "task-index",
          Array.from(taskUl.children).indexOf(task)
        );
      }
    });
    ...
    taskUl.addEventListener("drop", (event) => {
      ...
      if (targetTask && targetTask.tagName === "LI") {
        // Ensure it's a task
